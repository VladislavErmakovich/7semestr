...
typedef int* (*WAVREAD)(char* filename, WAVInfo& info);
typedef void (*WAVWRITE)(char* filename, WAVInfo& info, int* data);
typedef double(*Calculate)(float* signal, WAVInfo& info);

int main()
{
    HINSTANCE hDLL = LoadLibrary(L"rms_library.dll");
    if (!hDLL) {
        std::cerr << "Error loading DLL: " << GetLastError() << std::endl;
    }

    WAVREAD wavread = (WAVREAD)GetProcAddress(hDLL,
        "wavread");
    WAVWRITE wavwrite =
        (WAVWRITE)GetProcAddress(hDLL, "wavwrite");
    Calculate calculate =
        (Calculate)GetProcAddress(hDLL, "calculate");
    if (!wavread || !wavwrite || !calculate) {
        std::cerr << "Error getting function pointers" << std::endl;
        FreeLibrary(hDLL);
    }
    // Тестируем функции
